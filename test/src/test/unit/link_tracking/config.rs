...
fn can_set_policy() {
    let mut config: Config<&str, ()> = Config::default();
    config.cobs.set_policy(TypeName::Wildcard, Policy::Deny);
...
fn can_set_pattern() {
    let mut config: Config<&str, ()> = Config::default();
    config.cobs.set_pattern(
        TypeName::Wildcard,
        Pattern::Objects(Some(()).into_iter().collect()),
    );
...
fn can_insert_objects() {
    ...
    config
        .cobs
        .insert_objects(TypeName::Type("discussion"), vec![5, 6, 7, 8]);
...
fn can_remove_objects() {
    ...
    config
        .cobs
        .remove_objects(TypeName::Type("discussion"), vec![1, 2, 4]);
