        ...
        match value {
            Value::Object(map) => {
                ...
                Ok(Self {
                    ...
                    pattern: Pattern::try_from(pattern.clone())?,
                })
            },
    ...
    <Cstring as TryInto<Id>>::Error: std::error::Error + Send + Sync + 'static,
{
    ...
    fn try_from(value: &Value) -> Result<Self, Self::Error> {
        let try_typename =
            |typename: &Cstring| match typename.as_str() {
                "*" => Ok(TypeName::Wildcard),
                _ => typename.clone().try_into().map(TypeName::Type).map_err(
                    |err: <Cstring as TryInto<Ty>>::Error| error::Cobs::TypeName(err.into()),
                ),
            };
        match value {
