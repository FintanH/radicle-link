    ...
    for<'a> &'a C: RefScan,
{
    ...
    let mut tracked = peek.tracked.into_keys().collect::<BTreeSet<_>>();
    ...
    // Clear sigref tips and rad/id tips so far. Fetch will ask the
    // remote to advertise sigrefs and rad/id's from the transitive
    // trackings, so we can inspect the state afterwards to see if we
    // got any.
    state.sigref_tips_mut().clear();
    state.id_tips_mut().clear();
    ...
    if !state.sigref_tips().is_empty() || !state.id_tips().is_empty() {
