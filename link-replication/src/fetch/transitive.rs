...
impl<T: AsRef<oid>> Negotiation for Transitive<T> {
    ...
    where
        R: Refdb + Odb,
    {
        // FIXME: this is the same as for `super::Fetch`, but the compiler
        // chokes at abstracting out an iterator yielding iterators
        let mut bld = BuildWantsHaves::default();

        for (remote_id, refs) in &self.signed_refs {
            for (name, tip) in &refs.refs {
