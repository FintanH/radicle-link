...
pub fn cjson_derive(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    let expanded = quote! {
        // The generated impl.
        impl #impl_generics link_canonical::json::Cjson for #name #ty_generics #where_clause {
...
// Add a bound `T: Cjson` to every type parameter T.
fn add_trait_bounds(mut generics: Generics) -> Generics {
    for param in &mut generics.params {
        if let GenericParam::Type(ref mut type_param) = *param {
            type_param
                .bounds
                .push(parse_quote!(link_canonical::json::Cjson));
        }
    }
    generics
}
...
fn cjson_named_fields(
    ...
    let mut val = std::collections::BTreeMap::new();
        let inner = link_canonical::json::Value::Object(std::collections::BTreeMap::from_iter(
         kvs.map(|(key, value)| {
...
fn cjson_unnamed_fields(ident: &Ident, names: impl Iterator<Item = Ident>) -> TokenStream {
    let vs = names.map(|name| {
        quote! {
        link_canonical::json::Cjson::into_cjson(#name)
        }
    });
    quote! {
    ...
    let mut val = std::collections::BTreeMap::new();
        ...
        let inner = link_canonical::json::Value::Array(std::collections::BTreeSet::from_iter(vs));
    val.insert(link_canonical::Cstring::from(stringify!(#ident)), inner);
         link_canonical::json::Value::Object(val)
    }
}
