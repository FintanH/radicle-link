    ...
    E: std::error::Error + Send + 'static,
{
    for seed in seeds {
        ...
        match client.replicate(from, urn.clone(), None).await {
            Ok(result) => report(reporter, format!("replication success: {:#?}", result)).await?,
            Err(err) => {
                report(
                    reporter,
    ...
    loop {
        match replies.next().await {
            ...
            Ok(Reply::Success { .. }) => {
                ...
                report(reporter, "successful_announcement").await?;
                return Ok(());
            },
            Ok(Reply::Error { msg, .. }) => {
    ...
    E: std::error::Error + Send + 'static,
{
    ...
    report(reporter, format!("signed refs {}", msg)).await?;
    Ok(Some(at.into()))
}
                ...
                while let Some(resp) = request.next().await {
                    match resp {
                        Ok(request_pull::Response::Success(s)) => {
                            ...
                            report(reporter, format!("{:#?}", s)).await?;
                            break;
                        },
