...
pub struct Args {
    /// List of bootstrap nodes for initial discovery. If no bootstrap nodes
    /// were provided, we will fall back to configured nodes. The max number of
    /// configured nodes used will be the maximum allowed peers in the
    /// membership parameters (default: 5).
