    ...
    E: ConnectPeer + Clone + Send + Sync + 'static,
{
    ...
    pub async fn request_pull(
        ...
        // NOTE: the Runner needs to be detached to allow this function to be called in
        // Send-friendly environments, ie. linkd-lib
        let endpoint = self.endpoint.clone();
        let spawner = self.spawner.clone();
        let paths = self.paths.clone();
        self.spawner
            .spawn(async move {
                match request_pull::Runner::new(spawner, endpoint, urn, remote_peer, addrs, paths)
                    .await
                {
                    Ok(mut runner) => {
                        while let Some(response) = runner.next().await {
                            if let Err(e) = tx.send(response).await {
                                tracing::trace!(err = %e, "request-pull receiver went away");
                                break;
                            }
                        }
                    },
